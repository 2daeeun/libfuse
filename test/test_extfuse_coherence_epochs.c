...
_Static_assert(FUSE_KERNEL_MINOR_VERSION == 48, "unexpected protocol minor");
...
enum negotiation_mode {
	...
	NEGOTIATE_INVALID_WBCACHE_RELEASE_BARRIER,
};
...
static void test_init(void *userdata, struct fuse_conn_info *conn)
{
	...
	if (state->negotiation == NEGOTIATE_WBCACHE_PASSTHROUGH ||
	    ...
	    state->negotiation == NEGOTIATE_INVALID_WBCACHE_RELEASE_BARRIER) {
		if (state->negotiation == NEGOTIATE_WBCACHE_PASSTHROUGH &&
		...
		if (!fuse_set_feature_flag(
			...
			fprintf(stderr,
				"failed to opt in to passthrough attr refresh\n");
			return;
		}
		if (state->negotiation ==
		    NEGOTIATE_INVALID_WBCACHE_RELEASE_BARRIER)
	...
	if (state->negotiation == NEGOTIATE_ALL &&
		...
		fprintf(stderr, "failed to opt in to advertised coherence epochs extensions\n");
		return;
	}
	conn->extfuse_prog_fd = TEST_PROG_FD;
}
		      ...
		      enum negotiation_mode negotiation)
{
	...
	if (negotiation == NEGOTIATE_WBCACHE_PASSTHROUGH ||
	    ...
	    negotiation == NEGOTIATE_INVALID_WBCACHE_RELEASE_BARRIER)
		flags |= FUSE_EXTFUSE_WBCACHE_PASSTHROUGH |
			 FUSE_EXTFUSE_PASSTHROUGH_ATTR_REFRESH;
	if (negotiation == NEGOTIATE_WBCACHE_PASSTHROUGH ||
	    negotiation == NEGOTIATE_INVALID_WBCACHE_RELEASE_BARRIER)
		flags |= FUSE_WRITEBACK_CACHE;
	if (negotiation == NEGOTIATE_INVALID_WBCACHE_RELEASE_BARRIER)
		flags |= FUSE_EXTFUSE_PASSTHROUGH_ATTR_RELEASE_BARRIER;
			    ...
			    enum negotiation_mode negotiation)
{
	...
	uint64_t flags;
	uint64_t expected_epoch_flags = 0;
	...
	if (negotiation != NEGOTIATE_NONE) {
		expected_epoch_flags = FUSE_EXTFUSE_COHERENCE_EPOCHS;
		if (negotiation == NEGOTIATE_ALL)
			expected_epoch_flags = all_epoch_flags;
		if ((flags & all_epoch_flags) != expected_epoch_flags ||
		    !(flags & FUSE_FS_EXTFUSE) ||
		    init->extfuse_prog_fd != TEST_PROG_FD) {
			fprintf(stderr, "coherence epochs INIT opt-in was not serialized\n");
			return 1;
		}
	...
	if (negotiation == NEGOTIATE_WBCACHE_PASSTHROUGH) {
		if (!(flags & FUSE_WRITEBACK_CACHE) ||
		    !(flags & FUSE_EXTFUSE_WBCACHE_PASSTHROUGH) ||
		    !(flags & FUSE_EXTFUSE_PASSTHROUGH_ATTR_REFRESH) ||
...
static int run_session(enum negotiation_mode negotiation)
{
	...
	char argv0[] = "extfuse-coherence-epochs-test";
	char *argv = argv0;
	...
	if (negotiation != NEGOTIATE_ALL) {
		if (negotiation == NEGOTIATE_CORE_ONLY &&
		    fuse_lowlevel_notify_inval_xattr(session, TEST_NODEID) !=
			    -ENOSYS) {
			fprintf(stderr,
				"core-only coherence epochs unexpectedly enabled xattr notify\n");
			goto out_session;
		}
		failed = 0;
		goto out_session;
...
int main(void)
{
	...
	failed |= run_session(NEGOTIATE_INVALID_WBCACHE_RELEASE_BARRIER);
	if (!failed)
